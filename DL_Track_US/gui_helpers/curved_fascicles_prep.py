...
def apo_to_contour_orientation_map(image):
    ...
    mid = width / 2
    new_X_UA = np.linspace(0, width - 1, width)  # Extrapolate x,y data using f function
    # new_X_UA = np.linspace(-200, 800, 5000)
    new_Y_UA = g(new_X_UA)
    # new_X_LA = np.linspace(-200, 800, 5000)
    new_X_LA = np.linspace(0, width - 1, width)  # Extrapolate x,y data using f function
    new_Y_LA = h(new_X_LA)
